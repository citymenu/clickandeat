...
public class RestaurantDeclinedHandler implements IWorkflowHandler {
    ...
    public String getWorkflowAction() {
        return OrderWorkflowEngine.ACTION_RESTAURANT_DECLINED;
    }

    @Override
    public Order handle(Order order, Map<String, Object> context) throws WorkflowException {

        if( !OrderWorkflowEngine.ORDER_STATUS_AWAITING_RESTAURANT.equals(order.getOrderStatus())) {
        ...
        try {
            ...
            LOGGER.error("Error sending confirmation of restaurant declining order to customer");
            throw new WorkflowException(ex);
        }

        order.setOrderStatus(OrderWorkflowEngine.ORDER_STATUS_RESTAURANT_DECLINED);
        return order;
    }
